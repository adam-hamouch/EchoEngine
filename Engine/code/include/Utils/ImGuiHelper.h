...
public:
	template <typename T>
	static T* ResourceSelector(const char* title, const std::vector<T*>& resources, T* currentResource)
	{
		static_assert(std::is_base_of<IResource, T>::value, "T doit hériter de IResource");

		if (resources.empty())
		{
			ImGui::Text("%s : Aucune ressource disponible", title);
			return nullptr;
		}
		...
		std::string label = currentResource ? currentResource->GetPath() : "Choose...";

		const char* previewLabel = label.c_str();
		T* selectedResource = nullptr;

		if (ImGui::BeginCombo(title, previewLabel))
		{
			for (int i = 0; i < resources.size(); ++i)
			{
				if (!resources[i]) continue;

				std::string path = resources[i]->GetPath();
